...
from ace.networks import proposal_network, energy_network, gru_network
...
class ACEModel(tf.keras.Model):
    ...
    def __init__(
        ...
        energy_regularization: float = 0.5,
        **kwargs,
    ):
        ...
        self._proposal_network = gru_network(
                num_features,
                GRU_units,
                1,
                context_units,
...
###########################################################################################
        x = inputs[0]
        # print(x)
        # print(observed_mask, "222")
        # print(tf.where(observed_mask == 0), "222")
        inds = tf.where(1.0 - observed_mask == 1)[:, -1]
        inds = tf.random.shuffle(inds)
        inds = tf.cast(inds, tf.int32)
        # inds = x.shape[1]
        proposal_dist_his = 1.0
        cur_x_o = x
        cur_observed_mask = tf.cast(observed_mask, tf.float32)
        # print(inds)
        # for j in inds:
        for j in range(x.shape[1]):
            proposal_dist, context = self._proposal_network(
                [cur_x_o, cur_observed_mask], training=training
            )
            proposal_dist_his = proposal_dist_his * proposal_dist
            update_inds = tf.stack([tf.repeat(j, 1)], axis=1)
            update_inds = tf.cast(update_inds, tf.int32)

            temp = tf.reshape(cur_observed_mask[:, j], (-1, 1))
            # cur_observed_mask = tf.tensor_scatter_nd_update(cur_observed_mask, update_inds,
            #                                                 tf.cast(tf.ones_like(temp), tf.float32))
            cur_observed_mask = update_columns(cur_observed_mask, update_inds, tf.cast(tf.ones_like(temp), tf.float32))
...
###########################################################################################
        ...
        #     temp = tf.reshape(cur_observed_mask[j, :], (1, -1))
        #     # print(update_inds)
        #     # print(tf.cast(tf.ones_like(temp), tf.float32))
        #     cur_observed_mask = tf.tensor_scatter_nd_update(cur_observed_mask, update_inds,
        #                                                     tf.cast(tf.ones_like(temp), tf.float32))
        #     # print(j)
        #     # print(cur_observed_mask)
        #     cur_x_o = x * cur_observed_mask
#############################################################################################################
    ...
    def train_step(self, data):
        ...
        keras.optimizers.Adam(learning_rate=0.001).minimize(loss, self.trainable_variables, tape=tape)
    ...
    # def _dim_proposal_GRU_sample(self, x_o, observed_mask, index):
